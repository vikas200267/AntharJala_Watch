  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '0 14px 16px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, background: C.grn15, border: `0.5px solid ${C.grn}25`, borderRadius: 8, padding: '5px 10px', marginBottom: 12 }}>
        <Wifi size={12} color={C.grn} />
        <span style={{ fontFamily: F.b, fontSize: 10, color: C.grn }}>Real-time sync active — WebSocket</span>
        <div style={{ marginLeft: 'auto', width: 5, height: 5, borderRadius: '50%', background: C.grn, animation: 'pulse 1.8s ease infinite' }} />
      </div>

      {/* Borewell gauge card */}
      <Card style={{ marginBottom: 12, animation: anim ? 'fadeUp 0.5s ease 0.05s both' : 'none', padding: '14px 16px' }}>
      ...
      {/* Recent readings */}
      <div style={{ fontFamily: F.b, fontSize: 10, color: C.txd, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 8 }}>Recent Readings</div>
      {readings.map((r, i) => {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '0 14px 10px' }}>
        <Card style={{ padding: '10px 13px' }}>
          <div style={{ fontFamily: F.b, fontSize: 9, color: C.txd, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 7 }}>Water Stress Index · 847 readings</div>
...
function AlertsScreen() {
  ...
  const filtered = filter === 'all' ? ALERTS : ALERTS.filter(a => a.lvl === filter);
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '0 14px 16px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 0 12px' }}>
        ...
        <Tag label={`${ALERTS.length} Active`} color={C.rd} />
      </div>
  ...
  const send = useCallback((text) => {
    ...
    setTimeout(() => {
      ...
      setMessages(m => [...m, {
        ...
        text: `Analyzing: "${msg.slice(0, 40)}${msg.length > 40 ? '...' : ''}"\n\nBased on real-time data across 847 Karnataka boreholes, here is my analysis with ${activeModel} reasoning and SageMaker ML ensemble support.`,
      }]);
    }, 2200);
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '0 14px 16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7, marginBottom: 12 }}>
        {[{ l: 'Avg Level', v: '21.4m', c: C.cy }, { l: 'Peak Yield', v: '3.2K L/h', c: C.grn }, { l: 'Stress Days', v: '47', c: C.am }].map(s => (
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        <ResponsiveContainer width="100%" height={110}>
          <LineChart data={waterTrend} margin={{ top: 4, right: 4, bottom: 0, left: -28 }}>
            <XAxis dataKey="m" tick={{ fill: C.txd, fontSize: 8, fontFamily: F.b }} axisLine={false} tickLine={false} />
      ...
      <Card style={{ marginBottom: 12 }}>
        ...
        <ResponsiveContainer width="100%" height={90}>
          <BarChart data={waterTrend.slice(-6)} margin={{ top: 2, right: 4, bottom: 0, left: -28 }}>
            <XAxis dataKey="m" tick={{ fill: C.txd, fontSize: 8, fontFamily: F.b }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: C.txd, fontSize: 8 }} axisLine={false} tickLine={false} />
            <Tooltip content={({ active, payload }) => active && payload?.length ? (
              <div style={{ background: C.s2, border: `0.5px solid ${C.bdm}`, borderRadius: 8, padding: '5px 9px' }}>
...
function CommunityScreen() {
  ...
  if (activeThread) {
    const post = POSTS.find(p => p.id === activeThread);
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: '0 14px 16px' }}>
      ...
      <div style={{ padding: '22px 0 16px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <div style={{ width: 76, height: 76, borderRadius: 22, background: C.cy15, border: `1px solid ${C.cy3}`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14, fontFamily: F.d, fontSize: 24, color: C.cy, fontWeight: 700, animation: 'pulseGlow 4s ease infinite' }}>RK</div>
...
const NAV = [
  ...
  { id: 'ai', icon: MessageSquare, label: 'AI Guide' },
...
function PhoneShell({ children }) {
  return (
    <div style={{
      width: 340, height: 720, borderRadius: 42, overflow: 'hidden',
      background: C.bg,
      border: `1px solid rgba(0,175,220,0.2)`,
      boxShadow: `0 0 0 1.5px rgba(0,0,0,0.9), 0 0 0 3px rgba(0,175,220,0.06), 0 50px 100px rgba(0,0,0,0.85), 0 0 80px rgba(0,120,180,0.07)`,
      position: 'relative', display: 'flex', flexDirection: 'column', flexShrink: 0,
    }}>
      {/* Status bar */}
      <div style={{ height: 44, flexShrink: 0, display: 'flex', alignItems: 'center', paddingLeft: 18, paddingRight: 18, background: 'rgba(3,8,18,0.95)', backdropFilter: 'blur(12px)', position: 'relative', zIndex: 10 }}>
        <span style={{ fontFamily: F.m, fontSize: 11, color: C.txm, letterSpacing: '0.04em' }}>9:41</span>
        <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', width: 96, height: 26, borderRadius: 13, background: '#000', border: '0.5px solid rgba(255,255,255,0.03)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7 }}>
          <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#0e0e0e', border: '0.5px solid rgba(255,255,255,0.05)' }} />
          <div style={{ width: 3, height: 3, borderRadius: '50%', background: `${C.cy}55`, animation: 'pulse 2.5s ease infinite' }} />
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 5, alignItems: 'center' }}>
          <Wifi size={11} color={C.txd} />
          <span style={{ fontFamily: F.m, fontSize: 9, color: C.txd, letterSpacing: '0.08em' }}>●●●●</span>
        </div>
      </div>
      ...
      {/* Home indicator */}
      <div style={{ height: 20, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(3,8,18,0.95)' }}>
        <div style={{ width: 84, height: 3.5, borderRadius: 2, background: C.txd }} />
      </div>
    </div>
  );
}
...
export default function AntharJalaWatch() {
  ...
  const mainNav = ['dashboard', 'map', 'alerts', 'ai', 'community'];
  ...
  return (
    <div style={{
      minHeight: '100vh', background: '#000c18',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      padding: '24px 16px', gap: 20, fontFamily: F.b,
      backgroundImage: `
        radial-gradient(ellipse 80% 60% at 50% 50%, rgba(0,40,80,0.5) 0%, transparent 65%),
        radial-gradient(ellipse 40% 40% at 20% 20%, rgba(0,60,100,0.2) 0%, transparent 50%)
      `,
    }}>
      <style>{GLOBAL_CSS}</style>

      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, animation: 'fadeUp 0.7s ease' }}>
        <div style={{ width: 30, height: 30, borderRadius: 9, background: C.cy15, border: `0.5px solid ${C.cy3}`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Droplets size={15} color={C.cy} />
        </div>
        <span style={{ fontFamily: F.d, fontSize: 13, color: C.cy, letterSpacing: '0.18em', textTransform: 'uppercase' }}>Anthar-Jala Watch</span>
        <span style={{ fontFamily: F.b, fontSize: 9, color: C.txd, background: C.g, border: `0.5px solid ${C.bd}`, borderRadius: 4, padding: '2px 7px' }}>UI Prototype</span>
      </div>
      ...
      {/* Extra screen pills */}
      {appState === 'app' && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'center', animation: 'fadeUp 0.6s ease 0.4s both' }}>
          {[
            { id: 'history', label: '📊 Analytics' },
            { id: 'profile', label: '👤 Profile' },
            { id: 'addreading', label: '➕ Log Reading' },
            { id: 'media', label: '📷 Media & Batch' },
          ].map(({ id, label }) => (
            <button key={id} onClick={() => setScreen(id)} style={{
              padding: '5px 14px', borderRadius: 20, cursor: 'pointer', transition: 'all 0.15s',
              background: screen === id ? C.cy15 : 'rgba(255,255,255,0.04)',
              border: `0.5px solid ${screen === id ? C.cy3 : 'rgba(0,175,220,0.12)'}`,
              color: screen === id ? C.cy : 'rgba(210,236,245,0.45)',
      ...
      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'center' }}>
        {['Jetpack Compose', 'MVVM + Clean Arch', 'Room + WorkManager', 'Retrofit + Flows', 'Material 3 Dynamic', 'WebSocket + Kotlin Coroutines'].map(t => (
          <span key={t} style={{
            fontFamily: F.m, fontSize: 8, color: 'rgba(0,195,235,0.45)',
            background: 'rgba(0,195,235,0.04)', border: '0.5px solid rgba(0,195,235,0.1)',
